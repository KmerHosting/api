...
alter table public.dashboard_api_keys
  alter column scopes set default array['account:read','services:read','domains:read','email:read','hosting:read','vps:read','convertsuite:read','convertsuite:process']::text[];

-- Upgrade only the legacy default scope set. Explicitly scoped customer keys stay unchanged.
update public.dashboard_api_keys
set scopes = array['account:read','services:read','domains:read','email:read','hosting:read','vps:read','convertsuite:read','convertsuite:process']::text[]
