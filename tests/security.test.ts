...
test("rejects revoked and expired OAuth tokens", () => {
  expect(() => assertActiveToken({ revoked_at: "2026-01-01T00:00:00Z", expires_at: "2099-01-01T00:00:00Z" })).toThrow(ApiError);
  expect(() => assertActiveToken({ revoked_at: null, expires_at: "2026-01-01T00:00:00Z" })).toThrow(ApiError);
});
