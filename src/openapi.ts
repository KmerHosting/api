...
export const openapi = {
  ...
  security: [{ bearerAuth: [] }],
  tags: [{ name: "Account" }, { name: "Services" }, { name: "Domains" }, { name: "Email Hosting" }, { name: "Shared Hosting" }, { name: "LXC VPS" }],
  paths: {
    ...
    "/v1/hosting/services/{serviceId}/stats": { get: { tags: ["Shared Hosting"], summary: "Get DirectAdmin service statistics", description: "Requires `hosting:read`. Only DirectAdmin services currently support detailed statistics.", parameters: [{ ...id, name: "serviceId" }], responses: { "200": response("Hosting statistics") } } },
    ...
    "/v1/vps/instances": { get: { tags: ["LXC VPS"], summary: "List owned LXC VPS instances", description: "Requires `vps:read`.", responses: { "200": response("VPS instances") } } },
    "/v1/vps/instances/{serviceId}": { get: { tags: ["LXC VPS"], summary: "Get VPS details", description: "Requires `vps:read`.", parameters: [{ ...id, name: "serviceId" }], responses: { "200": response("VPS details") } } },
    "/v1/vps/instances/{serviceId}/actions": { post: { tags: ["LXC VPS"], parameters: [{ ...id, name: "serviceId" }, { $ref: "#/components/parameters/IdempotencyKey" }], ...mutation("Start, stop, shut down or restart a VPS", "vps:write", { type: "object", required: ["action"], properties: { action: { type: "string", enum: ["start", "stop", "shutdown", "restart"] } } }) } },
    "/v1/vps/instances/{serviceId}/auto-renew": { put: { tags: ["LXC VPS"], parameters: [{ ...id, name: "serviceId" }, { $ref: "#/components/parameters/IdempotencyKey" }], ...mutation("Set VPS auto-renew", "vps:write", { type: "object", required: ["enabled"], properties: { enabled: { type: "boolean" } } }) } },
    "/v1/vps/instances/{serviceId}/snapshots": {
      get: { tags: ["LXC VPS"], summary: "List VPS snapshots", description: "Requires `vps:read`.", parameters: [{ ...id, name: "serviceId" }], responses: { "200": response("Snapshots") } },
      post: { tags: ["LXC VPS"], parameters: [{ ...id, name: "serviceId" }, { $ref: "#/components/parameters/IdempotencyKey" }], ...mutation("Create a VPS snapshot", "vps:snapshots:write", { type: "object", required: ["name"], properties: { name: { type: "string", minLength: 1, maxLength: 30, pattern: "^[A-Za-z0-9 -]+$" }, description: { type: "string", maxLength: 255 } } }) },
    },
    "/v1/vps/instances/{serviceId}/snapshots/{snapshotId}": {
      patch: { tags: ["LXC VPS"], parameters: [{ ...id, name: "serviceId" }, { in: "path", name: "snapshotId", required: true, schema: { type: "string", maxLength: 160 } }, { $ref: "#/components/parameters/IdempotencyKey" }], ...mutation("Update a VPS snapshot", "vps:snapshots:write", { type: "object", minProperties: 1, properties: { name: { type: "string", minLength: 1, maxLength: 30, pattern: "^[A-Za-z0-9 -]+$" }, description: { type: "string", maxLength: 255 } } }) },
      delete: { tags: ["LXC VPS"], parameters: [{ ...id, name: "serviceId" }, { in: "path", name: "snapshotId", required: true, schema: { type: "string", maxLength: 160 } }, { $ref: "#/components/parameters/IdempotencyKey" }], summary: "Delete a VPS snapshot", description: "Requires `vps:snapshots:write` and a unique `Idempotency-Key` header.", responses: { "200": response("Snapshot deleted"), "409": { $ref: "#/components/responses/Error" } } },
    },
  },
  components: {
