...
export const openapi = {
  ...
  paths: {
    ...
    "/v1/hosting/services/{serviceId}/panel-access": { post: { tags: ["Shared Hosting"], parameters: [{ ...id, name: "serviceId" }, { $ref: "#/components/parameters/IdempotencyKey" }], ...mutation("Create a temporary hosting panel link", "hosting:panel:access", { type: "object", properties: { target: { type: "string", enum: ["panel", "filemanager"], default: "panel" } } }, "Panel access link; for Free Hosting, successful creation records activity and can recover an inactivity-suspended service before its deletion deadline") } },
