...
export async function handle(request: Request, config: Config = loadConfig(), store?: ApiStore): Promise<Response> {
  ...
  try {
    ...
    if (kvm) {
      ...
      if (area === "password" && request.method === "POST") return await writeRoute(request, config, db, id, "kvm:credentials:write", "kvm.instances.password", path, async (principal, body) => { await requireOwnedService(db, principal, serviceId, "kvm"); return forwardMutation(config, db, principal, "kvm", "POST", "/reset-password", { serviceId, password: body.password }); });
      if (area === "renew" && request.method === "POST") return await writeRoute(request, config, db, id, "kvm:subscription:write", "kvm.instances.renew", path, async (principal, body) => { await requireOwnedService(db, principal, serviceId, "kvm"); return forwardMutation(config, db, principal, "kvm", "POST", "/renew", { serviceId, billingMonths: body.billingMonths }); });
      if (area === "cancel" && request.method === "POST") return await writeRoute(request, config, db, id, "kvm:subscription:write", "kvm.instances.cancel", path, async (principal) => { await requireOwnedService(db, principal, serviceId, "kvm"); return forwardMutation(config, db, principal, "kvm", "POST", "/cancel", { serviceId }); });
      if (area === "keep-service" && request.method === "POST") return await writeRoute(request, config, db, id, "kvm:subscription:write", "kvm.instances.keep_service", path, async (principal) => { await requireOwnedService(db, principal, serviceId, "kvm"); return forwardMutation(config, db, principal, "kvm", "POST", "/keep-service", { serviceId }); });
      ...
      if (area === "snapshots" && snapshotId === "rollback" && request.method === "POST") return await writeRoute(request, config, db, id, "kvm:snapshots:write", "kvm.snapshots.rollback", path, async (principal, body) => { await requireOwnedService(db, principal, serviceId, "kvm"); return forwardMutation(config, db, principal, "kvm", "POST", "/snapshots/rollback", { serviceId, snapshotId: body.snapshotId }); });
    }
